...
def train(i, original_imgs_path):
	...
	input_nc = in_c
	output_nc = in_c
	NestModel = NestFuse_light2_nodense(nb_filter, input_nc, output_nc, deepsupervision=False)

	if args.resume_nestfuse is not None:
		print('Resuming, initializing using weight from {}.'.format(args.resume_nestfuse))
		NestModel.load_state_dict(torch.load(args.resume_nestfuse))
	print(NestModel)
	optimizer = Adam(NestModel.parameters(), args.lr)
	...
	# creating save path
	temp_path_model = os.path.join(args.save_nest_model, args.ssim_path[i])
	...
	for e in tbar:
		...
		for batch in range(batches):
			...
			# get fusion image
			# encoder
			en = NestModel.encoder(img)
			# decoder
			outputs = NestModel.decoder_train(en)
			# resolution loss
			...
			if (batch + 1) % (200 * args.log_interval) == 0:
				...
				save_model_filename = args.ssim_path[i] + '/' + "Epoch_" + str(e) + "_iters_" + str(count) + "_" + \
									  str(time.ctime()).replace(' ', '_').replace(':', '_') + "_" + args.ssim_path[
										  i] + ".model"
				save_model_path = os.path.join(args.save_nest_model, save_model_filename)
				torch.save(NestModel.state_dict(), save_model_path)
				# save loss data
				# pixel loss
